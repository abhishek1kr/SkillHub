---
name: fv-sol-7-c4-uninitialized-proxy
description: Proxy contracts that use initialize() instead of constructors for setup can be left in an uninitialized state if the initializer is never called, or can be re-initialized if the initializer lacks a one-time-use guard. Either condition al...
category: general
---
