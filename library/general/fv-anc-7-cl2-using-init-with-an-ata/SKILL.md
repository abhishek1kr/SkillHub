---
name: fv-anc-7-cl2-using-init-with-an-ata
description: Using [account(init, associatedtoken::...)] on a token account fails if the ATA already exists, because init requires the account to be uninitialized. Any user who pre-creates the ATA before the instruction runs triggers a transaction fa...
category: general
---
