---
name: fv-anc-3-cl13-init-if-needed-without-reinit-guard
description: Anchor's initifneeded skips account creation if the account already exists, but it does not prevent an attacker from pre-creating the PDA with malicious initial state. Without a reinitialization guard in the instruction body, a subsequen...
category: general
---
