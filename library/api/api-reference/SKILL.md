---
name: api-reference
description: header = base64url(json.dumps({"alg": "HS256", "typ": "JWT"})) payload = base64url(json.dumps({"sub": "admin"})) signature = hmac.new(publickeybytes, f"{header}.{payload}", hashlib.sha256) token = f"{header}.{payload}.{base64url(signatur...
category: api
---
